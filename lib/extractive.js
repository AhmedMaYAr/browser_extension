...
// Structured NIH-style abstracts already carry their own "Results: ...", "Conclusions: ..."
// section labels — strip those so we don't double them up with our own labels below.
const SECTION_LABEL = /^(background|objective|purpose|aim|methods?|design|setting|participants|patients|materials?|intervention|measures?|results?|findings?|conclusions?|interpretation|discussion|significance|importance)s?\s*:\s*/i;
...
function splitSentences(text) {
  return text
    ...
    .trim()
    .split(/(?<=[.!?])\s+(?=[A-Z0-9])/)
    .map((s) => s.trim().replace(SECTION_LABEL, ''))
    .filter(Boolean);
}
